...
import { ChevronLeft, ChevronRight, CheckCircle, Circle, BookOpen, Menu } from 'lucide-react'
...
export default function ArticleView({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 lg:px-10 py-4 lg:py-10">
      {/* Topic Header */}
      <div className="flex items-start justify-between mb-6 lg:mb-10 gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            {onMenuClick && (
              <button
                onClick={onMenuClick}
                className="lg:hidden p-1.5 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 -ml-1.5"
                aria-label="Open topic list"
              >
                <Menu className="w-5 h-5" />
              </button>
            )}
            <span className="text-xs font-mono text-purple-500 dark:text-purple-400 tracking-wider uppercase">
              Topic {currentTopicIndex + 1} of {allTopics.length}
            </span>
          </div>
          <h1 className="text-xl lg:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            <BookOpen className="w-6 h-6 lg:w-8 lg:h-8 text-purple-500 shrink-0" />
      ...
      {/* Navigation */}
      <div className="flex items-center justify-between mt-10 lg:mt-16 pt-6 lg:pt-8 border-t border-gray-200 dark:border-slate-600 gap-4">
        {prevTopic ? (
          <button
            onClick={() => onNavigate(prevTopic.slug)}
            className="flex items-center gap-2 lg:gap-3 px-4 lg:px-6 py-2.5 lg:py-3 rounded-xl text-xs lg:text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-600 min-w-0 max-w-[45%]"
          >
        ...
        {nextTopic ? (
          <button
            ...
            className="flex items-center gap-2 lg:gap-3 px-4 lg:px-6 py-2.5 lg:py-3 rounded-xl text-xs lg:text-sm font-medium text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-950/20 transition-colors border border-purple-200 dark:border-purple-800 min-w-0 max-w-[45%]"
          >
